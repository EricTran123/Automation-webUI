...
public class CommunicationManagement {
    ...
    public WebElement kafkaservice() throws Exception{
        ...
        element = driver.findElement(getElements.getLocator("paas.communicationmanagement.kafkaservice"));
        return element;
    }
    //返回通信服务总览页面中sms服务“立即使用”按钮页面元素对象
    public WebElement sms() throws Exception{
        //调用ObjectMap中的getLocator方法获取配置文件中关于用户名的定位方式和定位表达式
        element = driver.findElement(getElements.getLocator("paas.communicationmanagement.sms"));
        return element;
    }
    //返回通信服务总览页面中email服务“立即使用”按钮页面元素对象
    public WebElement email() throws Exception{
        //调用ObjectMap中的getLocator方法获取配置文件中关于用户名的定位方式和定位表达式
        element = driver.findElement(getElements.getLocator("paas.communicationmanagement.email"));
        return element;
    }
}
